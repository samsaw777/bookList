...
class Book {
  constructor(bookName, bookAuthor, bookId) {
...
class UI {
  ...
  static addBookToList(book) {
    ...
    <td class="py-3 border-b border-gray-400 bg-white">${book.bookId}</td>
...
document.getElementById("bookForm").addEventListener("submit", (e) => {
  ...
  if (bookName === "" || bookAuthor === "" || bookId === "") {
    ...
    // Get the books from the local storage and check if book ID is already present in it.
    const books = LocalStore.getBooks();
    ...
    books.forEach((book, index) => {
      if (bookId === book.bookId) {
        UI.showAlert(
          "Cannot add book as the ID is present in the List!",
          "red"
        );
        isPresent = true;
      }
    ...
    if (!isPresent) {
      ...
      const newBook = new Book(bookName, bookAuthor, bookId);
